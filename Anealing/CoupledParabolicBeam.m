...
function [ Tms,dmp, Gphis, tel ] =CoupledParabolicBeam(X,alp,d,Tn,dmp)
...
Ps = LegP(Pr(1,1),x);
Qs = LegQ(Pr(1,1),x);
Mh1 = Mhlg(Pr(1,2),x,-0.5);
Mhl2 = Mhlg(Pr(1,2),-x,-0.5);
...
ModeL = A(1,1)*Ps(1,1)+A(2,1)*Qs(1,1)+A(3,1)*Mh1(1,1)+Mhl2(1,1);
    ...
    for i=1:Nx
        ...
        Gphis(i,j) = real(MPF(j)/ModeL*(A(1,1)*Ps(1,1)+A(2,1)*Qs(1,1)+A(3,1)*Mh1(1,1)+Mhl2(1,1)));
